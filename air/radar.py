...
for i in range(0,3):
	...
	hat_acc.append(csv['accuracy'][i+3])
	knn_acc.append(csv['accuracy'][i+6])
	knnadwin_acc.append(csv['accuracy'][i+9])

	vfdt_std.append(csv['std'][i])
	hat_std.append(csv['std'][i+3])
	knn_std.append(csv['std'][i+6])
	knnadwin_std.append(csv['std'][i+9])

	vfdt_training.append(csv['training'][i])
	hat_training.append(csv['training'][i+3])
	knn_training.append(csv['training'][i+6])
	knnadwin_training.append(csv['training'][i+9])

	vfdt_testing.append(csv['testing'][i])
	hat_testing.append(csv['testing'][i+3])
	knn_testing.append(csv['testing'][i+6])
	knnadwin_testing.append(csv['testing'][i+9])
	
	vfdt_size.append(csv['size'][i])
	hat_size.append(csv['size'][i+3])
	knn_size.append(csv['size'][i+6])
	knnadwin_size.append(csv['size'][i+9])
	
	vfdt_ram.append(csv['ram'][i])
	hat_ram.append(csv['ram'][i+3])
	knn_ram.append(csv['ram'][i+6])
	knnadwin_ram.append(csv['ram'][i+9])
	
	vfdt_cpu.append(csv['cpu'][i])
	hat_cpu.append(csv['cpu'][i+3])
	knn_cpu.append(csv['cpu'][i+6])
	knnadwin_cpu.append(csv['cpu'][i+9])
...
s = sum(vfdt_points_f)
values = vfdt_points_f
plt.title('VFDT:' + str(s), size=15, y=1.05)
